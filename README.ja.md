...
1.6.2では保存画像のシグナルチェーン表示と設定の再利用、詳細画面の文字組、長いチェーン名のループ表示を追加しました。元画像・撮影形式・解像度は維持します。
...
このソースは**1.6.2（versionCode 18）**です。オフラインの操作ガイドで撮影とFAULTの操作を確認できます。1.0.0からの更新時は旧エフェクト設定を初期化し、保存済みの作品は保持します。
