...
公開済みの基準版は**1.3.1（versionCode 12）**です。この開発ソースには端末推奨設定の改訂・ADVANCED OFF時の通常撮影・解像度連動音声を追加していますが、まだ未公開です。Kotlinへの移行とRAW処理の高速化を含みます。初回起動時にオフラインのチュートリアルを表示し、設定から再表示できます。1.0.0からの更新時は新しいFAULTモデルに合わせて旧エフェクト設定を初期化し、保存済みの作品は保持します。
