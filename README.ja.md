...
開発ブランチでは、初期値OFFの[実験的な端末連動](docs/EXPERIMENTAL_SIGNALS.ja.md)を追加しています。FAULT別の5入力感度と、ブレ・熱雑音・スミアを扱います。公開済み1.3.0には含まれません。
...
このソースは**1.3.0（versionCode 11）**です。Kotlinへの移行とRAW処理の高速化を含みます。初回起動時にオフラインのチュートリアルを表示し、設定から再表示できます。1.0.0からの更新時は新しいFAULTモデルに合わせて旧エフェクト設定を初期化し、保存済みの作品は保持します。
