...
開発版では画面回転・サイズ変更に応じたカメラ配置、写真・動画・TAPの排他選択アイコン、任意のメール下書きによる不具合報告を追加しています。[検証記録](docs/audit/ADAPTIVE_WINDOW_FEEDBACK.md)。

1.6.0では画面サイズに応じた撮影UI、保存形式の直接切り替え、更新した操作ガイドを利用できます。撮影画面を離れるか画面をロックすると録画を終了・保存します。カメラの故障表現、任意の素材入力TAP、LIVE変化、MEDIA／DISPLAYモデルを利用できます。実験機能・ADVANCED MODE・音声は初期状態でOFF、保存済みの選択は保持します。
...
このソースは**1.6.0（versionCode 16）**です。オフラインの操作ガイドで撮影とFAULTの操作を確認できます。1.0.0からの更新時は旧エフェクト設定を初期化し、保存済みの作品は保持します。
