...
1.7.4ではADVANCEDでのAnalog FPV切り替え時のクラッシュを修正し、設定やカメラ接続の変更後に古い値が詳細画面へ混ざることを防ぎます。
...
このソースは**1.7.4（versionCode 23）**です。オフラインの操作ガイドで撮影とFAULTの操作を確認できます。1.0.0からの更新時は旧エフェクト設定を初期化し、保存済みの作品は保持します。
