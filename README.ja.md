...
1.6.1ではGPU実性能と画面サイズに応じた推奨設定、LIGHTプレビュー、TAPの中央配置と録画連動再生を追加しました。既存設定と保存サイズは保持します。撮影画面を離れると録画を終了・保存します。
...
このソースは**1.6.1（versionCode 17）**です。オフラインの操作ガイドで撮影とFAULTの操作を確認できます。1.0.0からの更新時は旧エフェクト設定を初期化し、保存済みの作品は保持します。
