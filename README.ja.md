...
1.7.5では写真を通じた効果設定の受け渡し、任意の診断情報コピー、写真ビューアの整理、ランダムやLIVEを説明するガイドの刷新を行いました。動画モードの確認振動も無効にしました。
...
このソースは**1.7.5（versionCode 24）**です。オフラインの操作ガイドで撮影とFAULTの操作を確認できます。1.0.0からの更新時は旧エフェクト設定を初期化し、保存済みの作品は保持します。
