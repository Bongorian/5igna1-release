...
1.3.1では、初期値OFFの[実験的な端末連動](docs/EXPERIMENTAL_SIGNALS.ja.md)を追加しています。FAULT別の5入力感度と、ブレ・熱雑音・スミアを扱います。実験機能・ADVANCED MODE・音声の初期値はOFFで、保存済み設定は保持します。
...
このソースは**1.3.1（versionCode 12）**です。Kotlinへの移行とRAW処理の高速化を含みます。初回起動時にオフラインのチュートリアルを表示し、設定から再表示できます。1.0.0からの更新時は新しいFAULTモデルに合わせて旧エフェクト設定を初期化し、保存済みの作品は保持します。
...
1.1.0のクローズドテストを実施中です。2026-09-09に同じAlphaトラックへ1.3.0の更新を審査送信しました。承認と管理対象の公開操作を待っています。[提出記録](docs/PLAY_1_3_0.md)。製品版の正式公開後にストアリンクを掲載します。
