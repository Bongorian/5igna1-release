...
1.7.0では画面回転・サイズ変更に応じたカメラ配置、写真・動画・TAPの排他選択アイコン、任意のメール下書きによる不具合報告を追加しています。[検証記録](docs/audit/ADAPTIVE_WINDOW_FEEDBACK.md)。

1.7.0では物理カメラの選択、デジタルズームを使わない光学調整、TAPの元音声保持、保存動画の録画開始時設定の参照・再利用に対応しました。
...
このソースは**1.7.0（versionCode 19）**です。オフラインの操作ガイドで撮影とFAULTの操作を確認できます。1.0.0からの更新時は旧エフェクト設定を初期化し、保存済みの作品は保持します。
