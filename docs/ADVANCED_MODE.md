...
**SEED** is the signed 64-bit structural identity. **EVENT SEED** is available for ROW ERROR, BIT ERROR, ADDRESS ERROR, BLOCK ERROR, the digital STREAM ERROR model, incident-generating MEDIA models and Network display. AUTO derives a stable identity from the structural SEED; a fixed seed reproduces incident scheduling given the same fault time, inputs and settings. It does not reproduce a changing camera image. SIGNAL `identitySeed` and `eventSeed` are reduced RGB rendering values, distinct from these full integer identities. Bayer RAW also uses the structural identity.
