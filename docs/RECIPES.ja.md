...
撮影画面のシャッフルを長押ししてRESEEDすると、操作値を保って故障位置やbiasが変わります。すぐ再抽選せず、その個体をしばらく観察してください。撮影した写真にも効果の設定が記録されます。写真を見る画面のSIGNALから確認し、後の撮影に使えます。[写真を通じた設定の受け渡し](USAGE.ja.md)。

変化が見えないときはLEVEL、FAULTの有効化、各操作値を確認します。STREAM ERRORのデジタル伝送は事故の間、静かになる設計です。[操作](USAGE.ja.md)
