...
Camera permission is used for previews and capture. Microphone permission is requested only for recording video with audio enabled. You can record silently with audio off. Recording takes place in the foreground and stops when you leave the app.

LIVE FAULT varies effects using random values and time generated inside the app. It does not read the microphone, motion sensors, temperature or CPU load for this feature.
