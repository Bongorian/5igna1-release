...
DISPLAYではCRT、Digital thru、Network display、LED displayを選択します。アナログMEDIAの後のDigital thruはピクセル保持または線形補間でアップコンバートします。Networkは実際の更新回数と途中解像度を下げ、次の受信まで画像を保持します。ノイズは重ねません。停止機会（OFFまたは2〜30 FAULT秒ごと）、停止区間（0.1〜1.5 FAULT秒）、更新目標（1〜30 fpsまたは入力と同じ）、解像度（縦横それぞれ20〜100%）を設定します。初期FAULT 0.55では、既定の適用値は12 fps・縦横78%・5秒ごとの機会・0.7秒の停止区間・55%の確率です。現在の適用目標を画面に表示します。自動停止にはLIVEが必要で、停止区間はLIVEの速度に従います。受信には独立した単調な時計を使い、ECHOの古い画像時刻では停止を解除しません。履歴・SEED・互換性は[時間モデル2](TIME_MODEL.ja.md)を参照してください。LEDは素子間隔、正方形のモジュール故障、明示的な更新周波数によるモジュール単位のちらつきを表現します。実際の通信やパケット改変は行いません。すべてRGB用で、処理済みRAWでは迂回します。保存画像の寸法は選択中の撮影形式に従います。

## アナログFPVの色ノイズを調整する

チェーン編集で **STREAM ERROR → 伝送モデル → アナログFPV** を選びます。MEDIA・DISPLAYより前に、受信映像のような色ノイズ・干渉・水平同期の揺れを加えます。

「受信品質」を上げると映像がクリアになります。「干渉の強さ」「干渉帯の幅」「色ノイズ」「色ノイズの大きさ」「同期の揺れ」で、乱れ方を調整できます。大きな色の塊を動かすには「色ノイズの大きさ」を上げます。0%では細かい色ノイズになります。受信品質100%・干渉0%で効果を外せます。モデルを切り替えても各モデルの操作値は保持します。

LIVE OFFでもノイズは動きます。動きを止めるにはLIVEをONにして一時停止、速さを変えるにはLIVEの調整画面を使います。[LIVEの操作](LIVE_FAULT.ja.md)。撮影画面・JPG・通常動画で使えるRGB上の効果で、RAW画素には適用しません。
