...
IDENTITYは故障位置やbias、MOTIONは連続的なdriftや位相、EVENTは一時的な事故です。時間が進んでもChainと個体seedを変えません。RESEEDは選択中の故障個体だけを変更し、操作値は保持します。COLOR MAPは個体seedを使いません。リセットは操作値だけを初期化します。
