...
The shuffle icon creates a combination of 2–5 faults with random controls. Hold the button to keep the current route and controls and reseed only its identities. LIVE couples the device to the chosen system: motion can disturb readout/tracking, for example. The faults have their own time behavior even with LIVE off. [LIVE settings](LIVE_FAULT.md).
