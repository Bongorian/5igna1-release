...
DISPLAY selects CRT, Digital thru, Network display or LED display. After analog MEDIA, Digital thru offers nearest-pixel or linear upconversion. Network display reduces the actual delivered frame rate and intermediate resolution, retaining the last received frame between updates. It adds no synthetic noise overlay. Its four controls are freeze interval (OFF or 2–30 seconds between freeze starts), freeze duration (0.1–1.5 seconds), frame rate limit (1–30 fps or source rate), and delivered resolution (20–100% of each side). Defaults at full FAULT are every 5 seconds, 0.7 seconds, 12 fps and 60% per side. The seed offsets the repeating freeze schedule. Freezes require LIVE and follow its speed, pause and reverse; frame rate reduction follows source timestamps and still works with LIVE off. Lower FAULT shortens freezes, raises the frame rate limit toward 60 fps and reduces resolution loss; zero FAULT bypasses all three. The rate is an upper limit and cannot exceed the input rate. A held frame survives resolution edits until delivery resumes. Old saved settings receive the new Network defaults; their CRT/LED controls remain intact. LED display models module pitch, square failed modules and module-level refresh flicker. Neither network model sends network traffic or modifies real packets. All these profiles remain RGB-only; processed RAW bypasses them. The saved output dimensions remain those of the selected capture format.
