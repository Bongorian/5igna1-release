...
DISPLAY selects CRT, Digital thru, Network display or LED display. After analog MEDIA, Digital thru offers nearest-pixel or linear upconversion. Network display reduces actual frame delivery and intermediate resolution, retaining received pixels between updates without adding synthetic noise. The controls specify opportunities every 2–30 fault seconds (or OFF), a 0.1–1.5 fault-second freeze window, a 1–30 fps delivery target (or source rate), and 20–100% per-side resolution. At initial FAULT 0.55, defaults apply 12 fps, 78% per side, 5-second opportunities, a 0.7-second window and 55% chance. The editor shows effective targets. LIVE is required for automatic freezes; its speed changes fault-time windows. Delivery follows a separate monotonic clock, so ECHO content age cannot release a stall. See [clock model 2](TIME_MODEL.md) for history, seeds and compatibility. LED display models module pitch, square failed modules and module-level refresh flicker with an explicit refresh frequency. Neither network model sends network traffic or modifies real packets. All these profiles remain RGB-only; processed RAW bypasses them. The saved output dimensions remain those of the selected capture format.

## Adjust Analog FPV color noise

In the chain editor choose **STREAM ERROR → Transmission model → Analog FPV**. It adds receiver-like colored noise, interference and horizontal sync disturbance before MEDIA and DISPLAY.

Raise reception quality for a clearer image. Adjust interference, band width, color noise, color noise size and sync disturbance to shape the damage. Raise color noise size for broad moving patches; 0% gives fine noise. Reception quality 100% with interference 0% bypasses the effect. Switching models retains each model’s controls.

Noise moves with LIVE OFF. To hold the pattern, turn LIVE ON and use Pause; to change its speed, open LIVE adjustments. [LIVE controls](LIVE_FAULT.md). This RGB effect applies to the viewfinder, JPG and ordinary video, not RAW samples.
