...
Original RAW photographs and original RAW video bypass processing. The existing DNG-sequence recorder remains bounded and timestamp matched. Future processed RAW recording can use the common fault-node snapshots; it is not currently offered. Color/media/display points require other signal representations and cannot be stored as valid processed Bayer data.
