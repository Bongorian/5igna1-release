...
The preview contains media selection and, for videos, Play/Pause. These control the source independently of the shutter: the shutter saves an image or starts/stops recording the processed video. Pausing a source can still record the current frame with evolving FAULT. Video loops while playing. Source audio is not imported; the existing optional microphone recording controls remain available. Input resolution is bounded and output preserves its aspect ratio. Unsupported files report a load failure. Selection is local to this session; original files are never modified.

Leaving the app stops ordinary preview and imported-video playback. Only an output recording may continue in the background, using a foreground service; its notification offers Stop when notification permission is granted. Finishing/stopping it releases the service and wake lock. Image processing and chunked photo writes wait while hidden and resume when the app returns.
