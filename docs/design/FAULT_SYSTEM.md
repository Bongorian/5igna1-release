...
Draft preview uses the same time coordinate. Editing is disabled during recording to preserve apply/cancel semantics and the shared displayed/recorded image contract. A saved JPEG captures what was displayed, including a visible draft. Camera close or reconfiguration invalidates retained frames. Buffers and readback are bounded and released on teardown.
