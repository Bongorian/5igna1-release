...
internal class TransportTargetProbe(private val source: String, private val supportsExternal: Boolean) {
    ...
    private fun renderTransport(texture: Int, oes: Boolean, transform: FloatArray?, frame: EffectState.Frame,
        ...
        for (node in frame.nodes) {
            ...
            // Investigation only: benchmark callers own a distinct full-size RGBA8 target.
            if (node === frame.nodes.last() && !network && ow == w && oh == h) {
                drawStage(input, first && oes, if (first) requireNotNull(transform) else IDENTITY,
                    node, if (node.id == Effects.CRT && kind == 3) ow else iw,
                    if (node.id == Effects.CRT && kind == 3) oh else ih, target, ow, oh)
