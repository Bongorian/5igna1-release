...
internal object FaultSensitivity {
    ...
    fun native(id: Int, source: Int): Boolean = when (source) {
    ...
    fun values(parameters: EffectParameters, id: Int, experimental: Boolean): FloatArray =
        if (!experimental || keys.none { parameters.manual(id, it) }) defaults[id]
        else FloatArray(5) { parameters.resolved(id, keys[it], initial(id, it)) }
}
