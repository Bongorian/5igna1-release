...
final class MediaThumbnail extends FrameLayout {
    ...
    CancellationSignal cancellation;int revision;boolean disposed;volatile boolean hasThumbnail;
    MediaThumbnail(MainActivity a){super(a);activity=a;setBackground(a.bg(MainActivity.PANEL,12,0xff394039));setClipToOutline(true);setClickable(true);
        image=new ImageView(a);image.setScaleType(ImageView.ScaleType.CENTER_CROP);addView(image,new LayoutParams(-1,-1));
        badge=a.text("",8,MainActivity.WHITE);badge.setGravity(Gravity.CENTER);badge.setBackground(a.bg(0xCC0A0C0D,4,0));LayoutParams bp=new LayoutParams(-2,a.dp(17),Gravity.BOTTOM|Gravity.END);bp.setMargins(0,0,a.dp(3),a.dp(3));badge.setPadding(a.dp(4),0,a.dp(4),0);addView(badge,bp);placeholder();
    }
    ...
    void load(Uri uri,boolean video){if(disposed)return;int ticket=++revision;if(cancellation!=null)cancellation.cancel();placeholder();if(uri==null)return;
        ...
        worker.execute(()->{Bitmap bitmap=null;String type=null;try{type=activity.getContentResolver().getType(uri);bitmap=activity.getContentResolver().loadThumbnail(uri,new Size(192,192),signal);}catch(Exception ignored){}
            Bitmap result=bitmap;boolean sequence="application/zip".equals(type);boolean raw=sequence||(type!=null&&type.contains("dng"));activity.handler.post(()->{if(disposed||ticket!=revision){if(result!=null)result.recycle();return;}
                if(result!=null){hasThumbnail=true;image.setScaleType(ImageView.ScaleType.CENTER_CROP);image.setImageBitmap(result);}
